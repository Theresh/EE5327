...
class ThreeLayerNet(object):
    ...
    def momentum_train(self, X, y=None,iters=10,momentum=0.9,learning_rate=1e-4,reg=0.25,batch_size=128):
        N,D=X.shape
        loss_his=[]# store training loss values
        train_acc_his=[] # store training accuracy
        ...
        for j in range(iters):
            ...
            X_batch = None
            y_batch = None
            index = np.random.choice(np.arange(N), size=batch_size)
            X_batch = X[index]
            y_batch = y[index]
            
            loss,grads,h3=self.losses( X_batch, y_batch, reg=0.25)# loss and gradient calculation
            g=np.append(g,loss)
            ...
            if j%10==0:
                train_acc = (np.argmax(h3,1) == np.argmax(y_batch,1)).mean()
    ...
    # nesterov
    ###############################################################################################
    def nesterov_train(self, X, y=None,iters=10,momentum=0.9,learning_rate=1e-4,reg=0.25,batch_size=128):
        N,D=X.shape
        loss_his=[]# store training loss values
        train_acc_his=[] # store training accuracy
        iterations_per_epoch = max(N / batch_size, 1)
    ...
    def adagrad_train(self, X, y=None,iters=10,momentum=0.9,batch_size=128,learning_rate=1e-4,reg=0.25,delta=1e-7):
        N,D=X.shape
        loss_his=[]# store training loss values
        train_acc_his=[] # store training accuracy
        iterations_per_epoch = max(N / batch_size, 1)
        ...
        for j in range(iters):
            ...
            loss,grads,h3=self.losses( X_batch, y_batch, reg=0.0)# loss and gradient calculation
            g=np.append(g,loss) 
    ...
    def rmsprop_train(self, X, y=None,iters=10,momentum=0.9,batch_size=128,learning_rate=1e-4,reg=0.25,delta=1e-7,decay_rate=0.999):
        N,D=X.shape
        loss_his=[]# store training loss values
        train_acc_his=[] # store training accuracy
        g=[]
    ...
    # Adam
    ###############################################################################################
    def adam(self, X, y=None,iters=10,momentum=0.9,learning_rate=1e-4,batch_size=128,reg=0.25,delta=1e-8,decay_rate1=0.9,decay_rate2=0.999):
        N,D=X.shape
        loss_his=[]# store training loss values
        train_acc_his=[] # store training accuracy
        g=[]
...
plt.title('MOMENTUM')
plt.plot(np.arange(0,200,10),loss_momentum,'-',label='MOMENTUM')

plt.title('NESTEROV')
plt.plot(np.arange(0,200,10),loss_nesterov,'-',label='nesterov')

plt.title('AdaGrad')
plt.plot(np.arange(0,200,10),loss_adagrad,'-',label='adagrad')

plt.title('RMSprop')
plt.plot(np.arange(0,200,10),loss_rmsprop,'-',label='RMSprop')

plt.title('Adam')
plt.plot(np.arange(0,200,10),loss_adam,'-',label='adam')
...
#plt.title('MOMENTUM')
plt.plot(np.arange(0,100,1),h_m,label='MOMENTUM')

#plt.title('NESTEROV')
plt.plot(np.arange(0,100,1),h_n,label='nesterov')

#plt.title('AdaGrad')
plt.plot(np.arange(0,100,1),h_ada,label='adagrad')

#plt.title('RMSprop')
#plt.plot(np.arange(0,100,10),loss_rmsprop,label='RMSprop')

#plt.title('Adam')
plt.plot(np.arange(0,100,1),h_adam,label='adam')
plt.xlim((0,100))
plt.xlabel('epoch')
plt.ylabel('training loss')
plt.legend()
